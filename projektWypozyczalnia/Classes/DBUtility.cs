...
public static class DBUtility
{
    ...
    public static void AddShutteringSystemToDatabase(string nameOfShuttering, string manufacturer, int length, List<int> widths)
    {
        try
        {
            using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
            {
                ...
                string checkSystemQuery = @"SELECT SystemID 
                                            FROM SystemOfShuttering 
                                            WHERE NameOfShuttering = @NameOfShuttering;";
                int systemId = -1;
                ...
                if (systemId == -1)
                {
                    string insertSystemQuery = @"INSERT INTO SystemOfShuttering (NameOfShuttering, Manufacturer) 
                                                 VALUES (@NameOfShuttering, @Manufacturer);";
                ...
                string checkLengthQuery = @"SELECT LengthID 
                                            FROM LengthCategory 
                ...
                if (lengthId == -1)
                {
                    string insertLengthQuery = @"INSERT INTO LengthCategory (SystemID, Length) 
                                                 VALUES (@SystemID, @Length);";
                ...
                string checkProductQuery = @"SELECT COUNT(*) 
                                             FROM ShutteringProduct 
                                             WHERE LengthID = @LengthID AND Width = @Width;";
                
                string insertProductQuery = @"INSERT INTO ShutteringProduct (LengthID, Width, AmountInStock, PriceOfShuttering) 
                                              VALUES (@LengthID, @Width, @AmountInStock, @PriceOfShuttering);";
    ...
    public static void AddEquipmentToDatabase(string nameOfEquipment)
    {
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            try
            {
                ...
                string query = @"INSERT INTO Equipment (NameOfEquipment, AmountInStock, PriceOfEquipment) 
    ...
    public static List<ShutteringSystem> GetShutteringSystems()
    {
        ...
        {
            Console.WriteLine("Błąd podczas ładowania systemów: " + e.Message);
        }
    ...
    public static List<LengthForSystem> GetLengthsForSystem(string systemName)
    {
        ...
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            try
            {
                ...
                string query = @"SELECT DISTINCT l.Length 
            ...
            {
                Console.WriteLine("Błąd podczas ładowania długości: " + e.Message);
            }
        }
    ...
    public static List<WidthForLength> GetWidthsForLength(int length)
    {
        ...
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            ...
            {
                Console.WriteLine("Błąd podczas ładowania szerokości i ilości: " + e.Message);
            }
        }
    ...
    public static Dictionary<int, (string Name, int AmountInStock)> GetEquipmentList()
    {
        ...
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            try
            {
                ...
                string query = @"SELECT EquipmentID, NameOfEquipment, AmountInStock 
                                 FROM Equipment;";
            ...
            {
                Console.WriteLine("Błąd podczas ładowania listy osprzętu: " + e.Message);
            }
        }
    ...
    public static List<string> GetAllEquipment()
    {
        ...
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            try
            {
                ...
                string query = @"SELECT NameOfEquipment
                                 FROM Equipment;";
            ...
            {
                Console.WriteLine("Błąd podczas łączenia się z bazą danych: " + e.Message);
            }
        }
    ...
    public static void AddShutteringToStock(string systemName, int length, int width, int amount)
    {
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            try
            {
                ...
                string query = @"UPDATE ShutteringProduct
    ...
    public static List<ShutteringStockItem> GetShutteringStockData()
    {
        ...
        { try
            { 
                connection.Open();
                string query = @"SELECT s.NameOfShuttering AS Name, s.Manufacturer, l.Length, p.Width, p.AmountInStock AS Amount
                                 FROM ShutteringProduct p
                                 INNER JOIN LengthCategory l ON p.LengthID = l.LengthID
                                 INNER JOIN SystemOfShuttering s ON l.SystemID = s.SystemID
    ...
    public static List<EquipmentStockItem> GetEquipmentStockData()
    {
        ...
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            ...
            {
                connection.Open();
                string query = @"SELECT e.NameOfEquipment AS EquipmentName, e.AmountInStock AS EquipmentAmount
                                 FROM Equipment e
    ...
    public static List<PriceEquipment> GetEquipmentPriceData()
    {
        ...
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            ...
            {
                connection.Open();
                string query = @"SELECT NameOfEquipment AS Name,PriceOfEquipment AS Price
                                 FROM Equipment;";
    ...
    public static void AddNewLending(int clientId, string startOfLendDate, string endOfLendDate, string addressOfBuilding, string? comment)
    {
        try
        {
            using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
            {
                ...
                string getMaxNumberQuery = @"SELECT NumberOfLend
                                             FROM Lending
                ...
                string insertQuery = @"INSERT INTO Lending (NumberOfLend, ClientID, StartLendDate, EndLendDate, Comment, AddressOfBuilding)
    ...
    public static List<RentalItems> GetAllLendings()
    {
        ...
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            try
            {
                ...
                WHERE 
                    strftime('%Y-%m-%d', substr(l.EndLendDate, 7, 4) || '-' || substr(l.EndLendDate, 4, 2) || '-' || substr(l.EndLendDate, 1, 2)) >= DATE('now');";
        ...
        using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
        {
            ...
            foreach (var item in items)
            {
                try
                {
                    ...
                    var checkProductQuery = @"SELECT COUNT(1) 
                                              FROM ShutteringProduct 
                    ...
                    var insertQuery = @"INSERT INTO LendingShutteringDetails 
                    ...
                    var updateQuery = @"UPDATE ShutteringProduct
                                        SET AmountInStock = AmountInStock - @Amount
                                        WHERE ProductID = @ProductID;";
        ...
        try
        {
            using (var connection = new SqliteConnection($"Data Source={DataBaseName}"))
            {
                ...
                string query = @"SELECT 
                                    ...
                                    lsd.Amount
                                FROM 
                                    LendingShutteringDetails lsd
                                INNER JOIN 
                                    Lending l ON lsd.LendingID = l.LendID
                                WHERE 
                                    strftime('%Y-%m-%d', substr(l.EndLendDate, 7, 4) || '-' || substr(l.EndLendDate, 4, 2) || '-' || substr(l.EndLendDate, 1, 2)) < DATE('now');";
                ...
                using (var command = new SqliteCommand(query, connection))
                {
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            var updateLendingShutteringQuery = @"UPDATE LendingShutteringDetails
                                                                SET Amount = 0
                                                                WHERE ProductID = @ProductID;
                            ";
